...
void stop_writeSound(RecodeInfo_t *recodeInfo)
{
    ...
    // if (recodeInfo->buffer != NULL)
    // {
    //     // free(recodeInfo->buffer);
    //     recodeInfo->buffer = NULL; // 중복 해제를 방지하기 위해 NULL로 설정
    // }
    // snd_pcm_drain(recodeInfo->handle);
    // snd_pcm_close(recodeInfo->handle);
